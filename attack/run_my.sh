#!/bin/bash

exp_name=LLaDA-8B-Base-pretrained-mimir-github-4_12_1.0e-5_10_512

for config_name in config_all ; do

# for exp_ratio in 1 0.5 2; do

# for div_type in r t m n; do

output_dir=./attack_results/${exp_name}/${config_name}
mkdir -p ${output_dir}
CUDA_VISIBLE_DEVICES=1 SAMA_METADATA_DIR=$output_dir \
    python -m attack.run \
    -c attack/configs/${config_name}.yaml \
    --output ${output_dir} \
    --base-dir /mnt/data/yuhongyao/paper_codes/difftext/SAMA/outputs/${exp_name} \
    --cache-dir ./attack_results/${exp_name}/cache \
    2>&1 | tee ${output_dir}/attack.log

done



exp_name=LLaDA-8B-Base-pretrained-mimir-github-4_12_1.0e-5_10_512

for config_name in config_samaminus; do

for exp_ratio in 0.5 2; do

for div_type in r t m n; do

output_dir=./attack_results/${exp_name}/${config_name}_${div_type}_${exp_ratio}
mkdir -p ${output_dir}
CUDA_VISIBLE_DEVICES=1 SAMA_METADATA_DIR=$output_dir \
    SAMA_EXP_RATIO=$exp_ratio SAMA_DIV_TYPE=$div_type  python -m attack.run \
    -c attack/configs/${config_name}.yaml \
    --output ${output_dir} \
    --base-dir /mnt/data/yuhongyao/paper_codes/difftext/SAMA/outputs/${exp_name} \
    --cache-dir ./attack_results/${exp_name}/cache \
    2>&1 | tee ${output_dir}/attack.log

done

done
done