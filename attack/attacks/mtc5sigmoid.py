...
# MTC2 + mask for token weight + sigmoid
class Mtc5sigmoidAttack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.sigmoid_scales = [1, 10, 25, 50, 75, 100, 125, 150, 200, 400, 600]
    ...
    def run(self, dataset: Dataset) -> Dataset:
        ...
        for scale in self.sigmoid_scales:
            scale = str(scale)
            scale_results = [m[scale] for m in membership_scores]
            scale_dataset = dataset.add_column(self.name, scale_results)
            results[scale] = scale_dataset
    ...
    def _compute_batch_scores(self, texts, batch_start_idx):
        ...
        for b in range(B):
            if len(step_scores[b]) == 0:
                raise Exception("No steps found for batch {}".format(b))
                batch_scores.append({str(scale): 0.0 for scale in self.sigmoid_scales})
            else:
                # weights = 1.0 / (np.arange(len(step_scores[b])) + 1)
                # weights = weights / weights.sum()
                # final_score = float(np.average(step_scores[b], weights=weights))

                step_score = step_scores[b]
                final_scores = {}
                for scale in self.sigmoid_scales:
                    scale = str(scale)
                    step_scale_results = np.array([d[scale] for d in step_score])
                    final_score = float(np.mean(step_scale_results))
                    final_scores[scale] = final_score

                batch_scores.append(final_scores)

                if self.save_metadata:
                    batch_metadata[b]["final_score"] = final_scores
    ...
    def _sigmoid_at_scale_result(self, ref, target):
        return {
            str(scale): float((1 / (1 + np.exp(-scale * (ref - target)))).mean())
            for scale in self.sigmoid_scales
        }
    ...
    def _subset_binary_comparison_with_metadata(
        ...
        if s <= 0:
            return (
                self._sigmoid_at_scale_result(ref_losses.mean(), target_losses.mean()),
                [],
            )
        ...
        t_sel = target_losses[idx_matrix].mean(axis=1)
        r_sel = ref_losses[idx_matrix].mean(axis=1)
        comparisons = self._sigmoid_at_scale_result(r_sel, t_sel)
        ...
        return comparisons, subset_details
