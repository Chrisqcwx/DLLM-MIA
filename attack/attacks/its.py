...
class ItsAttack(AbstractAttack):
    """
    ITS: Information-Theoretic Subset membership attack for diffusion language models.

    Detects training membership by comparing target vs reference model losses across
    progressive masking configurations, where token positions to mask are sampled
    using an attention-weighted graph sampler (``sample_engine``) that couples
    internal connectivity (alpha) with cross-subset diversity (lambda_penalty).

    Args:
        name: Attack instance name (used as the output column name).
        model: Fine-tuned diffusion model to test.
        tokenizer: Tokenizer paired with ``model``.
        config: Configuration dict (see ``attack/configs/config_its4-p0.1.yaml``
            for an example).
        device: Torch device to run on.

    Key config keys:
        steps (int): Number of progressive masking steps.
        subset_size (int): Tokens per subset vote.
        num_subsets (int): Subsets sampled per step.
        min_mask_frac (float): Starting mask fraction (default 0.05).
        max_mask_frac (float): Ending mask fraction (default 0.50).
        l_schedule (str): "linear" or "geometric".
        weight_start_layer_ratio / weight_end_layer_ratio (float):
            Range of reference-model attention layers used to build the
            token-sampling graph.
        sample_alpha (float): Internal-connectivity coefficient in the sampler
            (positive = repel, negative = attract, 0 = random).
        lambda_penalty (float): Cross-subset diversity penalty (default 0.1).
        least_m (int): Tie-breaking window when picking the next token.
        reference_model_path (str): HF id or path of the reference DLM
            (e.g. "GSAI-ML/LLaDA-8B-Base" or "Dream-org/Dream-v0-Base-7B").

    Returns:
        Dataset with the membership-score column added (values in [0, 1];
        higher indicates more likely member).

    Algorithm:
        1. Pull the reference model's attention pattern on the unmasked input
           to build a token-token adjacency matrix.
        2. For each step, sample ``num_subsets`` subsets of ``subset_size``
           tokens via ``sample_engine`` (graph sampler with anti-coupling).
        3. Mask those positions on both target and reference models; record
           token-level CE losses on the newly-masked positions.
        4. Compare per-subset reference-loss vs target-loss sums (binary vote);
           average across subsets and steps.
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.metadata_dir = os.path.join(
            self.metadata_dir,
            f"its_metadata_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
        )
