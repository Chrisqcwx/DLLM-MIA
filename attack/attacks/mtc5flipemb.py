...
class Mtc5flipembAttack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.emb_flip_ratio = float(
            config.get("emb_flip_ratio", 0.0)
        )  # New param for v6
    ...
    def _forward_loss(
        ...
        input_embeddings = masked_input_embeddings - self.emb_flip_ratio * (
            ori_input_embeddings - masked_input_embeddings
        )

        out = model(
            **{embed_param_name: input_embeddings},
