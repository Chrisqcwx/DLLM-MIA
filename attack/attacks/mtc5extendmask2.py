...
class Mtc5extendmask2Attack(AbstractAttack):
    """
    ...
    def _get_extend_inputs(self, input_ids, mask_input_ids, attention_mask):
        # import torch

        input_ids, mask_input_ids, attention_mask = (
            input_ids[0],
            mask_input_ids[0],
            attention_mask[0],
        )
        ...
        # 1. 找 mask
        mask = mask_input_ids == self.target_mask_id

        # 2. 每个位置的重复次数
        repeat_times = torch.ones_like(mask_input_ids)
        repeat_times[mask] = 2

        # 3. 扩展 A 和 B
        input_ids_expanded = torch.repeat_interleave(input_ids, repeat_times)
        mask_input_ids_expanded = torch.repeat_interleave(mask_input_ids, repeat_times)
        attention_mask_expanded = torch.repeat_interleave(attention_mask, repeat_times)

        # 4. 计算扩展后 index
        # 原始 index
        idx = torch.arange(len(input_ids)).to(input_ids_expanded.device)

        # 每个位置之前一共插入了多少个额外元素
        offset = torch.cumsum(repeat_times - 1, dim=0)

        # 只取 mask 位置的最终 index
        ori_idx = idx[mask] + offset[mask] - 1

        return (
            input_ids_expanded.unsqueeze(0),
            mask_input_ids_expanded.unsqueeze(0),
            attention_mask_expanded.unsqueeze(0),
            ori_idx.unsqueeze(0),
        )
    ...
    def _compute_batch_scores(self, texts, batch_start_idx):
        ...
        for step in range(self.num_steps):
            ...
            with torch.no_grad():
                (
                    input_ids_extend,
                    masked_ids_target_extend,
                    attention_mask_extend,
                    ori_indices,
                ) = self._get_extend_inputs(
                    input_ids, masked_ids_target, attention_mask
                )
                out = self.model(
                    input_ids=masked_ids_target_extend,
                    attention_mask=(
                        attention_mask_extend if not self.target_shift_logits else None
                    ),
                )
                logits = out.logits if hasattr(out, 'logits') else out[0]
                ...
                cumulative_target_losses[new_mask] = (
                    ce[:, ori_indices[0]] - ce[:, ori_indices[0] + 1]
                )
            ...
            with torch.no_grad():
                (
                    input_ids_ref_extend,
                    masked_ids_ref_extend,
                    attention_mask_ref_extend,
                    ori_indices,
                ) = self._get_extend_inputs(
                    input_ids_ref, masked_ids_ref, attention_mask_ref
                )
                out_r = self.ref_model(
                    input_ids=masked_ids_ref_extend,
                    attention_mask=(
                        attention_mask_ref_extend if not self.ref_shift_logits else None
                    ),
                )
                ...
                cumulative_ref_losses[new_mask_ref] = (
                    ce_r[:, ori_indices[0]] - ce_r[:, ori_indices[0] + 1]
                )
