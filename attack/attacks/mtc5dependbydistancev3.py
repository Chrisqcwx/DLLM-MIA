...
def sample_engine(attn_weights, n_samples, k_list, alpha=1.0, lambda_penalty=2.0):
    ...
    dist = 0.5 * (attn_weights + attn_weights.t()).float()

    # 记录每个点在所有采样中被选中的总次数
    global_counts = torch.zeros(L, device=attn_weights.device)
    global_counts[:4] = 1e7  # 前四个点固定选择
    global_counts[-4:] = 1e7  # 后四个点固定选择
    all_results = []
...
class Mtc5dependbydistancev3Attack(AbstractAttack):
    """
