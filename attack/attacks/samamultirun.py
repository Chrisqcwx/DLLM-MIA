...
class SamamultirunAttack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.run_times = int(os.getenv("SAME_RUNTIMES", "1"))
        print("!!!!!!! run_times:", self.run_times)
    ...
    def _compute_batch_scores(self, texts, batch_start_idx):
        ...
        for _ in range(self.run_times):
            for step in range(self.num_steps):
                ...
                for b in range(B):
                    ...
                    t_losses = (
                        cumulative_target_losses[b][masked_positions]
                        .detach()
                        .cpu()
                        .numpy()
                    )
                    r_losses = (
                        cumulative_ref_losses[b][masked_positions]
                        .detach()
                        .cpu()
                        .numpy()
                    )

                    score, subset_details = (
                        self._subset_binary_comparison_with_metadata(
                            t_losses,
                            r_losses,
        ...
        for b in range(B):
            if len(step_scores[b]) == 0:
                ...
                weights = 1.0 / (np.arange(len(step_scores[b]) // self.run_times) + 1)
                weights = np.concatenate([weights] * self.run_times)
