...
class Mtc5dependbyembeddingAttack(AbstractAttack):
    """
    ...
    def _compute_weights(self, input_ids_ref, attention_mask_ref):
        """
        用 reference 模型的 token embedding 之间的 cosine similarity 作为位置关系权重。
        返回形状为 (Lb, Lb) 的相似度矩阵（对行做 L2 归一化后做点积，范围 [-1, 1]）。
        该矩阵会作为 sample_engine 的邻接矩阵使用：值越高表示两个 token 越相关。
        """
        B, L = input_ids_ref.shape
        real_L = attention_mask_ref.sum(dim=1)
        all_sims = []

        # 取出 reference 模型的 token embedding 层
        embed_layer = self.ref_model.get_input_embeddings()

        with torch.no_grad():
            # (B, L, D)
            token_embeds = embed_layer(input_ids_ref).float()

            for b in range(B):
                Lb = int(real_L[b].item())
                if Lb == 0:
                    all_sims.append(torch.zeros(0, 0))
                    continue
                # (Lb, D)
                valid_embeds = token_embeds[b, :Lb, :]
                # L2 normalize，按行做归一化
                valid_embeds = F.normalize(valid_embeds, p=2, dim=-1)
                # (Lb, Lb) 的 cosine similarity
                sim_matrix = valid_embeds @ valid_embeds.t()
                all_sims.append(sim_matrix.cpu())
        return all_sims
