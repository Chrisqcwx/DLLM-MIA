...
class ShuffleIndicesGenerator:

    def __init__(self, shape, valid_length, shuffle_num, device=None):
        self.B, self.T = shape
        self.shuffle_num = shuffle_num
        self.device = device if device is not None else valid_length.device

        if not torch.is_tensor(valid_length):
            valid_length = torch.tensor(valid_length, device=self.device)

        self.valid_length = valid_length

        base = (
            torch.arange(self.T, device=self.device).unsqueeze(0).expand(self.B, self.T)
        )
        ...
        for _ in range(shuffle_num):
            ...
            for b in range(self.B):
                L = int(self.valid_length[b])
                perm = torch.randperm(L, device=self.device)
                idx[b, :L] = idx[b, perm]
            ...
            inv.scatter_(
                1, idx, torch.arange(self.T, device=self.device).expand(self.B, self.T)
            )
            reverse.append(inv)

        self.forward_indices = torch.stack(forward)  # (shuffle_num, B, T)
        self.reverse_indices = torch.stack(reverse)  # (shuffle_num, B, T)

    def get_forward_indices(self, shuffle_num=None):
        if shuffle_num is None:
            return self.forward_indices
        return self.forward_indices[:shuffle_num]

    def get_reverse_indices(self, shuffle_num=None):
        if shuffle_num is None:
            return self.reverse_indices
        return self.reverse_indices[:shuffle_num]


class Mtc4Attack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.shuffle_num = int(config.get("shuffle_num", "kkkk"))
        self.shuffle_weight = float(config.get("shuffle_weight", "kkkk"))
    ...
    def _forward_and_loss(
        ...
        attention_mask,
        labels,
        shuffle_gen: ShuffleIndicesGenerator,
    ):
        ...
        for shuffle_forward, shuffle_reverse in zip(
            shuffle_gen.get_forward_indices(), shuffle_gen.get_reverse_indices()
        ):
            forward_ids = [input_ids[b, shuffle_forward[b]] for b in range(B)]
            forward_ids = torch.stack(forward_ids, dim=0)
            forward_labels = [labels[b, shuffle_forward[b]] for b in range(B)]
            forward_labels = torch.stack(forward_labels, dim=0)
            out_s = model(
            ...
            logits_s = out_s.logits if hasattr(out_s, 'logits') else out_s[0]
            if self.target_shift_logits:
            ...
            ce_s = (
                F.cross_entropy(
                    ...
                    forward_labels.view(-1),
                    reduction='none',
                )
                .view(B, seq_len)
                .float()
            )
            ce_s = torch.stack([ce_s[b, shuffle_reverse[b]] for b in range(B)], dim=0)
            ce += ce_s * self.shuffle_weight / self.shuffle_num
        return ce
