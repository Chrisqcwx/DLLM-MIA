...
class MtininrefAttack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.use_hidden_states1 = int(config.get("use_hidden_states1", -1))
        self.use_hidden_states2 = int(config.get("use_hidden_states2", -1))
    ...
    def _forward(self, model, inputs):
        ...
        hidden_state1 = hidden_states[self.use_hidden_states1]
        hidden_state2 = hidden_states[self.use_hidden_states2]
        ...
        if 'llada' in model.__class__.__name__.lower():
            if inner_model.config.weight_tying:
                logits1 = F.linear(hidden_state1, inner_model.transformer.wte.weight, None)  # type: ignore
                logits2 = F.linear(hidden_state2, inner_model.transformer.wte.weight, None)  # type: ignore
            else:
                logits1 = inner_model.transformer.ff_out(hidden_state1)  # type: ignore
                logits2 = inner_model.transformer.ff_out(hidden_state2)  # type: ignore
            if inner_model.config.scale_logits:
                logits1.mul_(1 / (inner_model.config.d_model) ** 0.5)
                logits2.mul_(1 / (inner_model.config.d_model) ** 0.5)
        else:
    ...
    def _compute_batch_scores(self, texts, batch_start_idx):
        ...
        for step in range(self.num_steps):
            ...
            with torch.no_grad():
                ...
                #     ),
                # )
                logits, logits_inner = self._forward(
                    self.model,
                    {
                        'input_ids': masked_ids_target,
                        'attention_mask': (
                ...
                if self.target_shift_logits:
                    ...
                    logits_inner = torch.cat(
                        [logits_inner[:, :1, :], logits_inner[:, :-1, :]], dim=1
                    )
                ...
                ce_inner = (
                    F.cross_entropy(
                        logits_inner.view(-1, logits_inner.size(-1)),
                        input_ids.view(-1),
                        reduction='none',
                    )
                    .view(B, seq_len)
                ...
                cumulative_target_losses[new_mask] = ce[new_mask] - ce_inner[new_mask]
            ...
            with torch.no_grad():
                ...
                #     ),
                # )
                logits_r, logits_r_inner = self._forward(
                ...
                if self.ref_shift_logits:
                    ...
                    logits_r_inner = torch.cat(
                        [logits_r_inner[:, :1, :], logits_r_inner[:, :-1, :]], dim=1
                    )
                ...
                ce_r_inner = (
                    F.cross_entropy(
                        logits_r_inner.view(-1, logits_r_inner.size(-1)),
                        input_ids_ref.view(-1),
                        reduction='none',
                    )
                    .view(B, seq_len)
                ...
                cumulative_ref_losses[new_mask_ref] = (
                    ce_r[new_mask_ref] - ce_r_inner[new_mask_ref]
                )
