...
from attack.attacks.third_party import get_token_freq


# MTC2 + pretrained token weight
class Mtc6Attack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        # self.token_weight_temperature = float(config.get("token_weight_temperature", 1))
    ...
    def _compute_token_weights(self, input_ids_ref, attention_mask_ref):
        token_freq = get_token_freq(self.model)
        # check NaN in token_freq
        if torch.isnan(token_freq).any():
            raise ValueError("NaN values in token_freq")

        weights = torch.zeros_like(input_ids_ref, dtype=torch.float32)
        B = len(input_ids_ref)
        for b in range(B):
            input_ids = input_ids_ref[b]
            input_token_freq = token_freq[input_ids]
            neglog_freq = -torch.log(input_token_freq.clamp(1e-4, 1))
            neglog_freq = neglog_freq.clamp_min(0)
            if attention_mask_ref is not None:
                neglog_freq = neglog_freq * attention_mask_ref[b]
            weights[b] = neglog_freq
        # check NaN
        if torch.isnan(weights).any():
            raise ValueError("NaN values in weights")
