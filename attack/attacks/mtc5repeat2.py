...
class Mtc5repeat2Attack(AbstractAttack):
    """
    ...
    def _forward_loss(
        ...
        B, L = input_ids.shape
        if concat_type == None:
            model_input_ids = masked_ids_target
            model_attention_mask = attention_mask
            st, ed = 0, L
        elif concat_type == 'prefix':
            model_input_ids = torch.cat([input_ids, masked_ids_target], dim=1)
            model_attention_mask = torch.cat([attention_mask, attention_mask], dim=1)
            st, ed = L, 2 * L
        elif concat_type == 'suffix':
            model_input_ids = torch.cat([masked_ids_target, input_ids], dim=1)
            model_attention_mask = torch.cat([attention_mask, attention_mask], dim=1)
            st, ed = 0, L
        elif concat_type == 'both':
            model_input_ids = torch.cat(
                [input_ids, masked_ids_target, input_ids], dim=1
            )
        ...
        for step in range(self.num_steps):
            ...
            with torch.no_grad():
                ...
                cumulative_target_losses[new_mask] = (
                    2 * ce[new_mask] - ce_concat[new_mask]
                )
            ...
            with torch.no_grad():
                ...
                cumulative_ref_losses[new_mask_ref] = (
                    2 * ce_r[new_mask_ref] - ce_r_concat[new_mask_ref]
                )
