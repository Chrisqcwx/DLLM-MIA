...
class Mtc5maxkssAttack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.maxkss = float(config.get("maxkss", 1.0))
    ...
    def _subset_binary_comparison_with_metadata(
        ...
        diff = r_sel - t_sel
        maxk_num = max(1, int(self.maxkss * len(diff)))
        topk_diff = np.partition(diff, -maxk_num)[-maxk_num:]
        comparisons = topk_diff > 0
