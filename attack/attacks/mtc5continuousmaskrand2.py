...
class Mtc5continuousmaskrand2Attack(AbstractAttack):
    """
    ...
    def _compute_batch_scores(self, texts, batch_start_idx):
        ...
        for step in range(self.num_steps):
            ...
            for b in range(B):
                ...
                for addnum in (to_add // 2, to_add - to_add // 2):
                    if addnum == 0:
                        continue
                    wsums = np.array(
                        [
                            w[i : i + addnum].sum().item()
                            for i in range(len(w) - addnum + 1)
                        ]
                    )

                    chosen_idx = self.rng.choice(
                        len(wsums), size=1, p=wsums / wsums.sum()
                    )[0]
                    # print(chosen_idx, addnum)
                    # chosen_idx = torch.from_numpy(chosen_idx).to(candidates.device)
                    chosen = candidates[chosen_idx : chosen_idx + addnum]
