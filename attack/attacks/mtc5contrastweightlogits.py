...
class Mtc5contrastweightlogitsAttack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.contrast_weight_max_ratio = int(
            config.get("contrast_weight_max_ratio", 1.0)
        )  # top-k for contrast weighting
    ...
    def run(self, dataset: Dataset) -> Dataset:
        ...
        dataset = self._add_membership_column_compat(dataset, membership_scores)
    ...
    def _add_membership_column_compat(
        self, dataset: Dataset, membership_scores: list[float]
    ) -> Dataset:
        """
        Add membership score column with a compatibility fallback for datasets versions
        where DatasetInfo.copy() fails on unknown keys (e.g. task_templates).
        """
        if len(membership_scores) != len(dataset):
            raise ValueError(
                f"membership_scores length mismatch: {len(membership_scores)} != {len(dataset)}"
            )
        ...
        try:
            ...
            # Fallback: rebuild dataset from existing columns to bypass info.copy().
            data_dict = {col: dataset[col] for col in dataset.column_names}
            data_dict[self.name] = membership_scores
            return Dataset.from_dict(data_dict, split=getattr(dataset, "split", None))
    ...
    def _compute_token_weights(self, input_ids_ref, attention_mask_ref):
        ...
        Ls = attention_mask_ref.sum(dim=1)  # 每个样本的有效长度
        mask_input_ids = torch.ones_like(input_ids_ref) * self.target_mask_id
        with torch.no_grad():
            out = self.model(
            ...
            ref_logits = out_ref.logits if hasattr(out_ref, 'logits') else out_ref[0]
            if self.ref_shift_logits:
                logits = torch.cat([logits[:, :1, :], logits[:, :-1, :]], dim=1)
                ref_logits = torch.cat(
                    [ref_logits[:, :1, :], ref_logits[:, :-1, :]], dim=1
                )

            label_logits = logits.gather(
                dim=-1, index=input_ids_ref.unsqueeze(-1)
            ).squeeze(-1)
        ...
        weights = (label_logits - label_ref_logits) * attention_mask_ref.float().to(
            self.ref_device
        )
        # weights = weights.clamp(min=0.0)  # 只关注 ref_loss > target_loss 的位置
        # only topk
        for b in range(B):
            retain_indices = torch.topk(
                weights[b], k=self.contrast_weight_max_ratio * Ls[b]
            ).indices
            mask = torch.zeros_like(weights[b])
            mask[retain_indices] = 1
            weights[b] = weights[b] * mask
            # set neg inf
            weights[b][weights[b] < 1e-8] = -torch.inf

        # 避免全零行
        weights = torch.softmax(weights, dim=1)  # 归一化成概率分布
