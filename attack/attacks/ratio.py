...
class RatioAttack(AbstractAttack):
    ...
    def __init__(self, name, model, tokenizer, config, device):
        ...
        self.reference_device = model.device  # torch.device('cpu')
    ...
    def _load_reference(self):
        ...
        #     self.config['reference_model_path'], trust_remote_code=True
        # )
        ref_model_path = self.config['reference_model_path']
        reference_model, reference_tokenizer, _ = ModelManager.init_model(
            ref_model_path, ref_model_path, self.reference_device
        )
        ...
        with torch.cuda.amp.autocast(enabled=False):
            try:
                ...
                loss = F.cross_entropy(
                    shift_logits, shift_targets.contiguous().view(-1), reduction="none"
                )
    ...
    def extract_features(self, batch):
        ...
        ratio_feature = [
            -t_loss - r_loss for t_loss, r_loss in zip(target_losses, reference_losses)
        ]
