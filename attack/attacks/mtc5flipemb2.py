...
from attack.attacks.utils import get_model_nll_params, get_model_embedding_layer

# from attack.run import init_model
from attack.misc.models import ModelManager


# MTC2 + mask for token weight + repeat prefix
class Mtc5flipemb2Attack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.emb_st_ratio = float(config.get("emb_st_ratio", None))  # New param for v6
        self.emb_ed_ratio = float(config.get("emb_ed_ratio", None))  # New param for v6
        if self.emb_st_ratio is None or self.emb_ed_ratio is None:
            raise ValueError(
                "emb_flip_ratio must be specified in config for MTC5FlipEmb2Attack"
            )
    ...
    # --------------------------- Internals -----------------------------
    def _compute_token_weights(self, input_ids_ref, attention_mask_ref):
        """
        ...
        with torch.no_grad():
            out = self.ref_model(
                input_ids=torch.ones_like(input_ids_ref) * self.target_mask_id,
                attention_mask=(
    ...
    def _forward_loss(
        self, model, input_ids, masked_ids_target, attention_mask, concat_type=None
    ):
        B, L = input_ids.shape
        B, seq_len = masked_ids_target.shape
        embedding_layer, embed_param_name = get_model_embedding_layer(model)
        ori_input_embeddings = embedding_layer(input_ids)  # (B, L, D)
        masked_input_embeddings = embedding_layer(masked_ids_target)  # (B, L, D)
        st_input_embeddings = masked_input_embeddings + self.emb_st_ratio * (
            ori_input_embeddings - masked_input_embeddings
        )
        ed_input_embeddings = masked_input_embeddings + self.emb_ed_ratio * (
            ori_input_embeddings - masked_input_embeddings
        )

        st_out = model(
            **{embed_param_name: st_input_embeddings},
            attention_mask=(attention_mask if not self.target_shift_logits else None),
        )
        ed_out = model(
            **{embed_param_name: ed_input_embeddings},
            attention_mask=(attention_mask if not self.target_shift_logits else None),
        )
        st_logits = st_out.logits if hasattr(st_out, 'logits') else st_out[0]
        ed_logits = ed_out.logits if hasattr(ed_out, 'logits') else ed_out[0]
        # logits = logits[:, st:ed, :]
        if self.target_shift_logits:
            st_logits = torch.cat([st_logits[:, :1, :], st_logits[:, :-1, :]], dim=1)
            ed_logits = torch.cat([ed_logits[:, 1:, :], ed_logits[:, -1:, :]], dim=1)

        st_ce = (
            F.cross_entropy(
                st_logits.view(-1, st_logits.size(-1)),
                input_ids.view(-1),
                reduction='none',
            )
            .view(B, seq_len)
        ...
        ed_ce = (
            F.cross_entropy(
                ed_logits.view(-1, ed_logits.size(-1)),
                input_ids.view(-1),
                reduction='none',
            )
            .view(B, seq_len)
    ...
    def _compute_batch_scores(self, texts, batch_start_idx):
        ...
        token_weights = self._compute_token_weights(input_ids, attention_mask)
        ...
        for step in range(self.num_steps):
            ...
            for b in range(B):
                ...
                unmasked_valid = attention_mask[b]
                ...
                w = token_weights[b][candidates]
                w = w / w.sum()
                chosen_idx = self.rng.choice(
                    len(candidates), size=to_add, replace=False, p=w.cpu().numpy()
                )
            ...
            with torch.no_grad():
                ...
                cumulative_target_losses[new_mask] = ce[
                    new_mask
                ]  # - ce_concat[new_mask]
            ...
            with torch.no_grad():
                ce_r = self._forward_loss(
                    self.ref_model,
                    input_ids_ref,
                    masked_ids_ref,
                    attention_mask_ref if not self.ref_shift_logits else None,
                )
                # ce_r_concat = self._forward_loss(
                #     self.ref_model,
                #     input_ids_ref,
                #     masked_ids_ref,
                #     attention_mask_ref if not self.ref_shift_logits else None,
                ...
                cumulative_ref_losses[new_mask_ref] = ce_r[
                    new_mask_ref
                ]  # - ce_r_concat[new_mask_ref]
