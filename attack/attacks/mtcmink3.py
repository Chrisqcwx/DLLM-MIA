...
class Mtcmink3Attack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.mink = float(config.get("mink", 0.85))
    ...
    def _subset_binary_comparison_with_metadata(
        ...
        if s <= 0:
            ...
            mink_num = int(len(diff) * self.mink)
            # the lowest k values in diff
            mink_diff = (
                diff if self.mink == 1.0 else np.partition(diff, mink_num)[:mink_num]
            )

            return float((mink_diff > 0).mean()), []
        ...
        diff = r_sel - t_sel
        mink_num = int(len(diff[0]) * self.mink)
        # the lowest k values in diff
        mink_diff = (
            diff
            if self.mink == 1.0
            else np.partition(diff, mink_num, axis=-1)[:, :mink_num]
        ).sum(axis=-1)
        # batch_scores = float((mink_diff > 0).mean())

        comparisons = mink_diff > 0
