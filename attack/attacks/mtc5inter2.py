...
from attack.attacks.utils import get_model_nll_params, get_model_final_norm_linear_layer
...
class Mtc5inter2Attack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.concat_type = config.get(
            "concat_type", None
        )  # None, 'prefix', 'suffix', 'both'
        self.inter_layer = int(config.get("inter_layer", -1))  # New param for v6

        self.hidden_ratio = float(config.get("hidden_ratio", 1))  # New param for v6
    ...
    def _forward_loss(
        ...
        norm, unembedding = get_model_final_norm_linear_layer(model)
        ...
        logits = out.logits if hasattr(out, 'logits') else out[0]
        hidden_states = out.hidden_states[self.inter_layer]
        if self.inter_layer != -1:
            hidden_states = norm(hidden_states)
        hidden_logits = unembedding(hidden_states)
        # logits = logits[:, st:ed, :]
        if self.target_shift_logits:
            hidden_logits = torch.cat(
                [hidden_logits[:, :1, :], hidden_logits[:, :-1, :]], dim=1
            )
            logits = torch.cat([logits[:, :1, :], logits[:, :-1, :]], dim=1)
        ...
        hidden_ce = (
            F.cross_entropy(
                hidden_logits.view(-1, hidden_logits.size(-1)),
                input_ids.view(-1),
                reduction='none',
            )
            .view(B, seq_len)
        ...
        return ce + self.hidden_ratio * (hidden_ce - ce)
