...
class MyplossupdateAttack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.cfg_mask_ratio = float(config.get("cfg_mask_ratio", 0.5))
        ...
        self.metadata_dir = os.path.join(
            self.metadata_dir,
            f"my_metadata_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
        )
    ...
    def _compute_batch_scores(self, texts, batch_start_idx):
        ...
        for step in range(self.num_steps):
            ...
            all_mask_tensor = torch.full_like(
                masked_ids_target, self.target_mask_id, device=masked_ids_target.device
            )

            part_mask_tensor = torch.where(
                torch.rand_like(all_mask_tensor, dtype=torch.float32)
                < self.cfg_mask_ratio,
                all_mask_tensor,
                masked_ids_target,
            )
            ...
            with torch.no_grad():
                ...
                mask_out = self.model(
                    input_ids=part_mask_tensor,
                    attention_mask=(
                        attention_mask if not self.target_shift_logits else None
                    ),
                )
                mask_logits = (
                    mask_out.logits if hasattr(mask_out, 'logits') else mask_out[0]
                )
                if self.target_shift_logits:
                    mask_logits = torch.cat(
                        [mask_logits[:, :1, :], mask_logits[:, :-1, :]], dim=1
                    )
                ...
                cumulative_target_losses[cumulative_mask] = (
                    ce[cumulative_mask] - mask_ce[cumulative_mask]
                )
            ...
            with torch.no_grad():
                ...
                cumulative_ref_losses[cumulative_mask_ref] = (
                    ce_r[cumulative_mask_ref] - mask_ce[cumulative_mask_ref]
                )
