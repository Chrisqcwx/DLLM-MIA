...
class Mtc5reffullprobAttack(AbstractAttack):
    """
    ...
    # --------------------------- Internals -----------------------------
    def _compute_token_weights(self, input_ids_ref, attention_mask_ref, model):
        """
        ...
        return weights, ce  # 返回 CPU 上的权重供后续采样
    ...
    def _compute_batch_scores(self, texts, batch_start_idx):
        ...
        token_weights, _ = self._compute_token_weights(
            input_ids_ref, attention_mask_ref, self.ref_model
        )
        # _, target_ce = self._compute_token_weights(
        #     input_ids, attention_mask, self.model
        # )

        ref_ce = self._forward_loss(
            self.ref_model,
            input_ids_ref,
            input_ids_ref,  # no mask for initial loss
            attention_mask,
        )
        target_ce = self._forward_loss(
            self.model,
            input_ids,
            input_ids,  # no mask for initial loss
            attention_mask,
        )
        ...
        for step in range(self.num_steps):
            ...
            with torch.no_grad():
                ...
                cumulative_target_losses[new_mask] = (
                    ce[new_mask] - target_ce[new_mask]
                )  # - ce_concat[new_mask]
            ...
            with torch.no_grad():
                ...
                cumulative_ref_losses[new_mask_ref] = (
                    ce_r[new_mask_ref] - ref_ce[new_mask_ref]
