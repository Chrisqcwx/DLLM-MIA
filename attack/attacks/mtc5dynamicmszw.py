...
class Mtc5dynamicmszwAttack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.subset_size_ratio = float(config.get("subset_size_ratio", 0.5))
    ...
    def _subset_binary_comparison_with_metadata(
        ...
        if m == 0:
            return 0.0, []
        s = max(subset_size, target_losses.shape[-1] * self.subset_size_ratio)
        s = min(subset_size, m)
        if s <= 0:
