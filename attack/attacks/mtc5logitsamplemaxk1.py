...
class Mtc5logitsamplemaxk1Attack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.maxk = float(config.get("maxk", 1))
    ...
    def _compute_token_weights(self, input_ids_ref, attention_mask_ref):
        ...
        with torch.no_grad():
            ...
            label_logits = (
                logits.gather(dim=-1, index=input_ids_ref.unsqueeze(-1))
                .squeeze(-1)
                .float()
                .clamp_min(0)
            )
        ...
        ce = label_logits * attention_mask_ref.float().to(self.ref_device)
        # 避免全零行
        ce = ce + 1e-8
    ...
    def _compute_batch_scores(self, texts, batch_start_idx):
        ...
        for step in range(self.num_steps):
            ...
            with torch.no_grad():
                ...
                cumulative_target_losses[new_mask] = label_logits[new_mask]
            ...
            with torch.no_grad():
                ...
                label_logits_r = (
                    logits_r.gather(dim=-1, index=input_ids.unsqueeze(-1))
                    .squeeze(-1)
                    .float()
                )

                new_mask_ref = new_mask.to(self.ref_device)
                cumulative_ref_losses[new_mask_ref] = label_logits_r[new_mask_ref]
    ...
    def _subset_binary_comparison_with_metadata(
        ...
        diff = target_losses - ref_losses
        maxk_num = max(1, int(len(diff) * self.maxk))

        m = maxk_num
        if m == 0:
            return 0.0, []
        s = min(subset_size, m)
        # the maxest k values in diff
        maxk_diff = (
            diff if self.maxk == 1.0 else np.partition(diff, -maxk_num)[-maxk_num:]
        )
        ...
        diff_sel = maxk_diff[idx_matrix].sum(axis=1)

        # t_sel = target_losses[idx_matrix]  # .sum(axis=1)
        # r_sel = ref_losses[idx_matrix]  # .sum(axis=1)
        # diff = t_sel - r_sel
        # maxk_num = int(len(diff[0]) * self.maxk)
        # # the maxest k values in diff
        # maxk_diff = (
        #     diff
        #     if self.maxk == 1.0
        #     else np.partition(diff, -maxk_num, axis=-1)[:, -maxk_num:]
        # )
        # maxk_diff = diff_sel
        comparisons = diff_sel > 0
