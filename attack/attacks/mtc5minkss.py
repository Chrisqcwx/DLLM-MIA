...
class Mtc5minkssAttack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.minkss = float(config.get("minkss", 1.0))
    ...
    def _subset_binary_comparison_with_metadata(
        ...
        diff = r_sel - t_sel
        minkk_num = max(1, int(self.minkss * len(diff)))
        lowest_k_diff = np.partition(diff, minkk_num - 1)[:minkk_num]
        comparisons = lowest_k_diff >= 0
