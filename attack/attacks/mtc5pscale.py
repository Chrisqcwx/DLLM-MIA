...
# MTC2 + mask for token weight + p ** scale
class Mtc5pscaleAttack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.p_scale = [0.1, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 2, 4, 8]
    ...
    def _compute_batch_scores(self, texts, batch_start_idx):
        ...
        for b in range(B):
            if len(step_scores[b]) == 0:
                ...
                batch_scores.append({str(scale): 0.0 for scale in self.p_scale})
            else:
    ...
    def _pscale_result(self, ref, target):
        return {
            str(scale): (
                (ref**scale).sum(axis=-1) > (target**scale).sum(axis=-1)
            ).mean()
            for scale in self.p_scale
        }
    ...
    def _subset_binary_comparison_with_metadata(
        ...
        if s <= 0:
            return (
                self._pscale_result(ref_losses, target_losses),
                [],
            )
        ...
        t_sel = target_losses[idx_matrix]
        r_sel = ref_losses[idx_matrix]
        comparisons = self._pscale_result(r_sel, t_sel)
