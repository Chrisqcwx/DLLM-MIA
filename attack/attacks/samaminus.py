...
class SamaminusAttack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.div_type = str(os.getenv("SAMA_DIV_TYPE", "n"))
        self.exp_ratio = float(os.getenv("SAMA_EXP_RATIO", "1.0"))

        print(f"!!!!!! div_type: {self.div_type} !!!!!!!")
        print(f"!!!!!! exp_ratio: {self.exp_ratio} !!!!!!!")
    ...
    def _get_div(self, r, t):
        match self.div_type:
            ...
            case "t":
                return (r - t) / (t + 1e-6)
            case "m":
                return (r - t) / ((r + t) / 2 + 1e-6)
            case _:
                raise ValueError(f"Invalid div_type: {self.div_type}")
    ...
    def _subset_binary_comparison_with_metadata(
        ...
        if s <= 0:
            return (
                float(
                    (
                        self._get_div(
                            (ref_losses**self.exp_ratio),
                            (target_losses**self.exp_ratio),
                        ).sum()
                    )
                ),
                [],
        ...
        t_sel = target_losses[idx_matrix] ** self.exp_ratio
        r_sel = ref_losses[idx_matrix] ** self.exp_ratio
        comparisons = self._get_div(r_sel, t_sel).sum(axis=1)

        t_sel = t_sel.sum(axis=1)
        r_sel = r_sel.sum(axis=1)
