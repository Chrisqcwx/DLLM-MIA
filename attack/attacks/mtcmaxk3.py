...
class Mtcmaxk3Attack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.maxk = float(config.get("maxk", 0.85))
    ...
    def _subset_binary_comparison_with_metadata(
        ...
        s = min(subset_size, m)
        if s <= 0:
            diff = ref_losses - target_losses
            maxk_num = int(len(diff) * self.maxk)
            # the lowest k values in diff
            maxk_diff = (
                diff if self.maxk == 1.0 else np.partition(diff, -maxk_num)[-maxk_num:]
            )

            return float((maxk_diff > 0).mean()), []
        ...
        # batch_scores = float((maxk_diff > 0).mean())

        comparisons = maxk_diff > 0
