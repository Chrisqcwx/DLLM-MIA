...
class UnembeddingSubspace:
    def __init__(self, lm_head_weight, k=256, m=8192, seed=42):
        """
        Args:
...
class MtaccessAttack(AbstractAttack):
    """
    ...
    def __init__(self, name: str, model, tokenizer, config, device: torch.device):
        ...
        self.use_hidden_states_start = int(config.get("use_hidden_states_start", -1))
        self.use_hidden_states_end = int(config.get("use_hidden_states_end", -1))
        ...
        if 'llada' in model.__class__.__name__.lower():
            if inner_model.config.weight_tying:
                unembedding_weight = inner_model.transformer.wte.weight
                # logits = F.linear(hidden_state, inner_model.transformer.wte.weight, None)  # type: ignore
            else:

                # logits = inner_model.transformer.ff_out(hidden_state)
                unembedding_weight = inner_model.transformer.ff_out.weight
        else:
            raise ValueError("Model type not supported")

        self.subspace = UnembeddingSubspace(unembedding_weight, seed=self.seed)
    ...
    def _forward(self, model, inputs):
        ...
        st = (
            self.use_hidden_states_start
            if self.use_hidden_states_start >= 0
            else self.use_hidden_states_start + len(hidden_states)
        )
        ed = (
            self.use_hidden_states_end
            if self.use_hidden_states_end >= 0
            else self.use_hidden_states_end + len(hidden_states)
        ) + 1

        hidden_state = hidden_states[st:ed]
        proj = self.subspace.compute_access_ratio(hidden_state).mean(dim=0)
        assert proj.ndim == 2, f"Expected 2D projection, got {proj.shape}"
        return proj
        ...
        if 'llada' in model.__class__.__name__.lower():
            if inner_model.config.weight_tying:
                ...
                logits = inner_model.transformer.ff_out(hidden_state)  # type: ignore
            if inner_model.config.scale_logits:
                logits.mul_(1 / (inner_model.config.d_model) ** 0.5)
        else:
            raise NotImplementedError("Only for llada")
