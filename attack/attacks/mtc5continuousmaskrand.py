...
class Mtc5continuousmaskrandAttack(AbstractAttack):
    """
    ...
    def _compute_batch_scores(self, texts, batch_start_idx):
        ...
        for step in range(self.num_steps):
            ...
            for b in range(B):
                ...
                # )
                wsums = np.array(
                    [w[i : i + to_add].sum().item() for i in range(len(w) - to_add + 1)]
                )

                chosen_idx = self.rng.choice(len(wsums), size=1, p=wsums / wsums.sum())[
                    0
                ]
                # chosen_idx = torch.from_numpy(chosen_idx).to(candidates.device)
                chosen = candidates[chosen_idx : chosen_idx + to_add]
