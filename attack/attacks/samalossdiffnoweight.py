...
class SamalossdiffnoweightAttack(AbstractAttack):
    """
    ...
    def _compute_batch_scores(self, texts, batch_start_idx):
        ...
        diff_target_losses = torch.zeros(
            B, seq_len, dtype=torch.float32, device=self.device
        )
        diff_ref_losses = torch.zeros(
            B, seq_len, dtype=torch.float32, device=self.ref_device
        )
        ...
        for step in range(self.num_steps):
            ...
            with torch.no_grad():
                ...
                if prev_mask is not None:
                    diff_target_losses[prev_mask] = (
                        ce[prev_mask] - cumulative_target_losses[prev_mask]
                    )
            ...
            with torch.no_grad():
                ...
                if prev_mask_ref is not None:
                    diff_ref_losses[prev_mask_ref] = (
                        ce_r[prev_mask_ref] - cumulative_ref_losses[prev_mask_ref]
                    )

            for b in range(B):
                if prev_mask is None:
                    continue
                masked_positions = torch.where(prev_mask[b])[0]
                m = int(masked_positions.numel())
                if m == 0:
                    continue

                t_losses = (
                    diff_target_losses[b][masked_positions].detach().cpu().numpy()
                )
                r_losses = diff_ref_losses[b][masked_positions].detach().cpu().numpy()
