...
class MIARunner:
    ...
    def __init__(self, args):
        ...
        self.config_manager = ConfigManager(args.config, args.cache_dir)
    ...
    def _initialize_model(self):
        ...
        model_path = (
            self.args.target_model if self.args.target_model else model_path_from_config
        )
        ...
        tokenizer_path = resolve_path(
            tokenizer_path, self.args.base_dir, load_from_base_dir
        )
    ...
    def run(self):
        ...
        header = ["MIA Attack", "AUC"] + [
            f"TPR@FPR={t}" for t in self.global_config["fpr_thresholds"]
        ]
        results_to_print = {}
        all_results = {}
        ...
        for ds_info in self.config_manager.get_datasets():
            ...
            all_results[ds_name] = attack_results

            # Store results
            results_to_print[ds_name] = tabulate(
                attack_results, headers=header, tablefmt="outline"
            )
            self._update_metadata(metadata, ds_name, attack_results, dataset, header)

        # Save and display results
        self._save_and_display_results(
            metadata, results_to_print, all_results=all_results
        )
    ...
    def _run_attacks_on_dataset(self, dataset, ds_name):
        ...
        for (
            attack_name,
            attack_module,
        ) in self.config_manager.get_available_attacks().items():
            ...
            if isinstance(processed_dataset, dict):
                ...
                for subname, subdataset in processed_dataset.items():
                    y_true = subdataset['label']
                    y_score = subdataset[attack_name]
                    ...
                    save_name = f"{attack_name}_{subname}"
                    attack_results.append([save_name] + metrics)
                    print(f"Attack {attack_name}_{subname} results: {metrics}")
            else:
    ...
    def _save_and_display_results(self, metadata, results_to_print, all_results=None):
        ...
        try:
            ...
            with open(
                os.path.join(self.args.output, metadata_filename[-4:] + '.json'), "w"
            ) as f:
                json.dump(all_results, f, indent=4, ensure_ascii=False)
        except Exception as e:
            print(f"!!!Error saving results: {e}", (e.__traceback__))
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run Membership Inference Attacks on Diffusion LLMs."
    )
    parser.add_argument(
        '-c',
        '--config',
