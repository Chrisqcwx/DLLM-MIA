...
from .utils2 import batch_nlloss
...
class DatasetProcessor:
    ...
    def __init__(
        self, model, tokenizer, device: torch.device, global_config: Dict[str, Any]
    ):
        ...
        self.cache_dir = global_config.get("cache_dir", None)
    ...
    def init_dataset(
        ...
        # Load dataset
        dataset = self._load_dataset(ds_info, base_dir=base_dir)
    ...
    def _compute_nll(
        ...
        if cache_dataset_file and os.path.exists(cache_dataset_file):
            dataset = torch.load(cache_dataset_file, weights_only=False)
            return dataset
...
def load_json_dataset(
    train_path: str, test_path: str, base_dir: Optional[str] = None
) -> Dataset:
