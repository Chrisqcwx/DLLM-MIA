...
class ModelManager:
    ...
    def get_model_nll_params(model) -> Tuple[bool, int]:
        ...
        if hasattr(unwrapped_model, 'config') and hasattr(
            unwrapped_model.config, 'model_type'
        ):
            model_type = unwrapped_model.config.model_type
            logging.info(
                f"Detected model_type: '{model_type}' from loaded model's config"
            )
            ...
            logging.warning(
                "Could not determine model_type from model config. Using defaults."
            )

        logging.info(
            f"NLL params: shift_logits={determined_shift_logits}, mask_id={determined_mask_id}"
        )
