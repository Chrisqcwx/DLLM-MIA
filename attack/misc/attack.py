...
def load_attack(
    ...
    class_name = (
        ''.join(word.capitalize().split("-")[0] for word in real_attack_name.split('_'))
        + 'Attack'
    )
    ...
    return attack_class(
        name=attack_name, model=model, tokenizer=tokenizer, config=config, device=device
    )
