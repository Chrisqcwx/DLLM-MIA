...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        if not output_attentions:
            ...
            attn_weights = attn_weights / torch.sqrt(q.size(-1))
        ...
        return self.attn_out(att), present, *info
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def forward(
        ...
        # Get attention scores.
        if self._activation_checkpoint_fn is not None:
            att, cache, *attn_infos = self._activation_checkpoint_fn(  # type: ignore
                self.attention,
                q,
                k,
                v,
            ...
            att, cache, *attn_infos = self.attention(
                q,
                k,
                v,
                attention_bias,
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                if (
                    ...
                    # shape: (batch_size, seq_len, d_model)
                    x, cache, *infos = self._activation_checkpoint_fn(
                        block,
                        x,
                        attention_bias=attention_bias,
                        layer_past=layer_past,
