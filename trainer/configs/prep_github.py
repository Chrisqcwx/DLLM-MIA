...
OUTPUT_CONFIG_DIR = "./ablations/epoch_num"
...
def generate_configs():
    ...
    print(
        f"🚀 Starting config generation for {len(MODELS)} models and {len(DATASETS)} datasets."
    )
    ...
    for model_id in MODELS:
        for dataset_name in DATASETS:
            for epoch in range(1, 11):
                ...
                TRAINING_PARAMS["num_train_epochs"] = epoch
                ...
                output_dir = f"./ablations/epoch_num/{run_name}-{output_dir_suffix}"

                # Construct dataset paths
                dataset_path_prefix = f"{BASE_DATASET_PATH}/{dataset_name}"
                ...
                # --- Write the file ---
                file_name = (
                    f"{run_name}-epoch{TRAINING_PARAMS['num_train_epochs']}.yaml"
                )
    ...
    print("-" * 40)
    print(
        f"🎉 Success! All {total_files} config files have been generated in the '{OUTPUT_CONFIG_DIR}' directory."
    )
