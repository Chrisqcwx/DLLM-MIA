...
os.environ["WANDB_MODE"] = (
    "offline"  # Ensure wandb runs in offline mode for this script
)
import wandb
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Train & track router signals in a MoE model."
    )
    parser.add_argument(
        "--config_path",
        type=str,
        default="./configs/LLaDA-115m-Base-raw-pretraining-lora-16-mimir-arxiv.yaml",
        help="Path to your config file (YAML).",
    )
    parser.add_argument(
        "--base_path", type=str, default="./", help="Base path for saving outputs."
    )
    parser.add_argument(
        "--train_subset_size", type=int, default=-1, help="Size of training subset."
    )
    parser.add_argument(
        "--ref_subset_size",
        type=int,
...
def load_and_prepare_datasets(config):
    ...
    if raw_test is None and raw_val is not None:
        logger.warning(
            "TEST dataset not found. Using VAL dataset as fallback for TEST."
        )
        raw_test = raw_val

    if raw_test is None and raw_val is None:
        raise ValueError(
            "Neither 'test' nor 'val' dataset is provided. Please specify at least one."
        )
...
def subset_and_save(
    ...
    raw_train, train_indices = subset_data(
        raw_train, train_subset_size, train_streaming
    )
...
def main():
    ...
    train_config = config["dataset"]["train"]
    test_config = (
        config["dataset"].get("test")
        if config["dataset"].get("test")
        else config["dataset"].get("val")
    )
    ...
    trainer = initialize_trainer(
        config, model, tokenized_train, tokenized_val, tokenizer, mask_id
    )
