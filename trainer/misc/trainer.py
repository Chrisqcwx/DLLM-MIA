...
try:
    ...
    LoraConfig, get_peft_model, prepare_model_for_kbit_training, TaskType, PeftModel = (
        None,
        None,
        None,
        None,
...
class DiffusionLLMTrainer(Trainer):
    ...
    def __init__(
        self, train_mode: str = "pretraining", mask_id: int = 126336, *args, **kwargs
    ):
        ...
        if self.train_mode not in ["pretraining", "sft"]:
            raise ValueError(
                f"train_mode must be 'pretraining' or 'sft', got {self.train_mode}"
            )
        ...
        if hasattr(model_to_check, 'config') and hasattr(
            model_to_check.config, 'model_type'
        ):
            actual_model_type_str = model_to_check.config.model_type
            logger.info(
                f"Detected model_type: '{actual_model_type_str}' from model config."
            )
    ...
    def compute_loss(
        ...
        **kwargs,
        ...
        if self.train_mode == "pretraining" and torch.rand(1).item() < 0.01:
            new_length = torch.randint(
                low=1, high=input_ids.shape[1] + 1, size=(1,)
            ).item()
            input_ids = input_ids[:, :new_length]

        noisy_batch, masked_indices, p_mask = forward_process(
            input_ids, mask_id=self.mask_id
        )
        ...
        # print(next(model.parameters()).device, noisy_batch.device)
        # exit()
        ...
        if masked_labels.numel() == 0:
            ...
            if not isinstance(loss, torch.Tensor):
                loss = torch.tensor(
                    loss, device=logits.device, requires_grad=model.training
                )
            elif model.training:
        ...
        if self.train_mode == "pretraining":
            ...
            answer_lengths = answer_mask.sum(dim=1).clamp(
                min=1
            )  # Number of answer tokens per sample, min 1 to avoid div by zero

            row_id = (
                torch.arange(bsz, device=masked_indices.device)
                .unsqueeze(1)
                .repeat(1, seqlen)
                .view(-1)
            )
            ...
            ans_len_for_masked = answer_lengths[
                row_id_masked
            ]  # Already clamped earlier
